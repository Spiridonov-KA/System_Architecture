...
// $lookup сделать нельзя, это aggregation запрос, они не поддерживаются
// поэтому хотя бы так
Poco::JSON::Array::Ptr Cart::expanded_items() const {
    Poco::JSON::Array::Ptr items = new Poco::JSON::Array();
    for (const std::string& item_id : _items) {
        std::optional<database::Item> item = database::Item::get_by_id(item_id);
        if (item) {
